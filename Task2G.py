...
def run():
    ...
    print("High Flood risk ( current relative water level > 0.8 ):" + str(nameshighf))
    mlist = stations_level_over_threshold(stations, 0.7)
    for k in mlist:
        namesmod.append(k[0])
    namesmodf = set(namesmod) - set(nameshigh)
    print("Moderate Flood risk ( current relative water level > 0.7 ):" + str(namesmodf))
    for station in stations:
        names.append(station.name)
    nameslow = set(names) - set(namesmod)
    print("Low Flood risk ( current relative water level <= 0.7 ):" + str(nameslow))
